...
interface EventDelegatorForCloning {
  clone: Widget;
  originalToClone: Widget;
  cloneToOriginal: Widget;
}
...
export default class Widget extends PropertyEventEmitter implements WidgetModel {
  ...
  protected _deepCloneProperties(clone: EventEmitter, properties: string[], options: CloneOptions) {
    if (!properties) {
      return clone;
    }
    properties = arrays.ensure(properties);
   ...
   */
  mirror(options?: CloneOptions, target?: EventEmitter) {
  ...
  protected _mirror(clone: EventEmitter, options: CloneOptions) {
  ...
  protected _unmirror(target: EventEmitter) {
