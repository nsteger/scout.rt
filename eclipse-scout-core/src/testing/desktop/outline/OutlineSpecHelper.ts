...
export default class OutlineSpecHelper {
  ...
  createModelFixture(nodeCount: number, depth: number, expanded: boolean) {
    return this.createModel(this.createModelNodes(nodeCount, depth, expanded));
  }

  createModel(nodes: RefModel<PageModel>[]) {
    let model = {
      objectType: Outline,
  ...
  createOutlineAdapter(model: ModelAdapterModel): OutlineAdapter {
