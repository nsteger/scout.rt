 ...
 */
import {Action, arrays, Column, Form, FormModel, GroupBox, Mode, ModeSelector, RadioButton, RadioButtonGroup, scout, Session, StringField, TabBox, TabItem, Table, TableField, Widget} from '../../index';
import $ from 'jquery';
...
export default class FormSpecHelper {
  ...
  createViewWithOneField(model: FormModel): Form {
    let form = this.createFormWithOneField(model);
    form.displayHint = Form.DisplayHint.VIEW;
    return form;
  }

  createFormWithOneField(model: FormModel): Form {
    let defaults = {
  ...
  createFieldModel(objectType: ObjectType, parent: Widget, modelProperties: Record<string, any>) {
    parent = scout.nvl(parent, this.session.desktop);
    let model = {
      objectType: objectType || StringField,
      parent: parent
    };
    if (modelProperties) {
  ...
  createField(objectType: ObjectType, parent: Widget, modelProperties: Record<string, any>) {
